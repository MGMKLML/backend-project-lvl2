...
const getChildren = (node) => Object.keys(node).slice().sort();
  ...
  const iter = (node, ancestry) => {
    const keys = getChildren(node).map((key) => {
      ...
      if (_.has(json1, path) && _.has(json2, path)) {
        ...
        if (before === after) {
          return {
            key, path, type: 'same', value,
          };
        }
        return {
          key, path, type: 'changed', before, after,
        };
      } if (_.has(json1, path) && !_.has(json2, path)) {
        return {
          key, path, type: 'removed', value,
        };
      }
      return {
        key, path, type: 'added', value,
      };
    });
    return keys;
  };
